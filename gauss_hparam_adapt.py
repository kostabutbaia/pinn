...
lambda_b = torch.tensor(1.0, requires_grad=True)
...
name = f'adaptive_lambda_{epochs}'
...
for epoch in range(epochs):
    ...
    # \hat{\lambda}_i update
    lambda_hat = grad_pde / (grad_boundary + 1e-8)
        
    # (b) Update lambda_i using moving average
    lambda_b = torch.tensor((1 - alpha) * lambda_b + alpha * lambda_hat.item(), requires_grad=True)
    ...
    if epoch % 100 == 0:
        ...
        print(f"Epoch {epoch}: Loss = {loss.item()}, lambda_b={lambda_b}")
